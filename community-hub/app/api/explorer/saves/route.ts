    ...
    try {
      ...
      if (championId) {
        res = await client.query(
          `SELECT id, champion_id, champion_data, vantheon_data, status, created_at, updated_at
           FROM explorer_saves
           WHERE champion_id = $1
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { id, champion, vantheon, status } = body;
    ...
    try {
      await client.query(
        `INSERT INTO explorer_saves (id, champion_id, champion_data, vantheon_data, status, updated_at)
        ...
        [
          ...
          vantheon ? JSON.stringify(vantheon) : null,
          status || "preparing",
        ],
      );
