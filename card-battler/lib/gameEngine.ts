...
export function drawCard(state: GameState): GameState {
  ...
  if (player.hand.length >= 5 || player.drawPile.length === 0) {
    newState.hasDrawn = true;
    newState.turnSubPhase = "move";
...
export function getValidMoves(
  ...
  return hexNeighbors(position).filter((n) => {
    ...
    // Allow swapping with a friendly unmoved champion
    return (
...
export function finishMovePhase(state: GameState): GameState {
  const newState = cloneState(state);
  newState.turnSubPhase = "action";
  return newState;
}
...
export function resolveAttack(
  ...
  newState.turnActionType = "attack";
...
export function playChampionFromHand(
  ...
  newState.turnActionType = "deploy";
...
// --- Transition from Action to Move2 ---

export function finishActionPhase(state: GameState): GameState {
  const newState = cloneState(state);
  newState.turnSubPhase = "move2";
  return newState;
}
...
export function endTurn(state: GameState): GameState {
  ...
  if (nextIdx >= newState.turnOrder.length) {
    ...
    // Skip eliminated players in the current round
    let found = false;
  ...
  if (roundEnded) {
    ...
    // Recompute turn order by score descending for subsequent rounds
  ...
  newState.hasActed = false;
  newState.turnActionType = null;
